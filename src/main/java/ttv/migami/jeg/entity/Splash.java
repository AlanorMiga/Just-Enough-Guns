...
public class Splash extends Entity {
    ...
    public Splash(Level pLevel, LivingEntity pPlayer, BlockPos pPos) {
        ...
        this.setPos(Vec3.atCenterOf(pPos.offset(0.5, -0.5, 0.5)));
    ...
    public void tick() {
        ...
        if (!level.isClientSide)
        {
                ...
                for (int y = -radius; y <= radius; y++) {
                    for (int z = -radius; z <= radius; z++) {
                        ...
                        if (blockState.getBlock() instanceof FireBlock) {
                            level.playSound(null, this.blockPosition(), SoundEvents.FIRE_EXTINGUISH, SoundSource.PLAYERS, 0.05F, 1F);
                ...
                if (entity instanceof LivingEntity && entity != owner) {
                    ...
                    if (this.getTextureIndex() == 3) {
                        if (livingEntity.getType().is(ModTags.Entities.FIRE)) {
                            livingEntity.hurt(new DamageSource("generic"), damage * 2);
                        }
                        else {
                            livingEntity.hurt(new DamageSource("generic"), damage);
                        }
                    }
            ...
            if (this.getTextureIndex() == 3 && tickCounter % 4 == 0 && this.tickCount > 20) {
                level.playSound(null, this.blockPosition(), SoundEvents.PLAYER_SPLASH_HIGH_SPEED, SoundSource.PLAYERS, 2F, 1F);
            }
