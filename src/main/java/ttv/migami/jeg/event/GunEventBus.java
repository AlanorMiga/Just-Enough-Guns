    ...
    public static void typhooneeBlast(Level level, Player player, float damage) {
        if(!level.isClientSide()) {
                ...
                for (int y = -radius; y <= radius; y++) {
                    for (int z = -radius; z <= radius; z++) {
                        ...
                        if (blockState.getBlock() instanceof FireBlock) {
                            ...
                            level.playSound(null, new BlockPos(result.getLocation()), SoundEvents.FIRE_EXTINGUISH, SoundSource.PLAYERS, 0.2F, 1F);
                        }
                    }
                }
            }
            ...
            if(e != null && e.getEntity() instanceof LivingEntity entity) {
                ...
                entity.hurt(new DamageSource("generic"), damage * advantageMultiplier);
