 ...
 */
public class DoubleBarrelShotgunModel implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrixStack, MultiBufferSource buffer, int light, int overlay) {
        ...
        RenderUtil.renderModel(SpecialModels.DOUBLE_BARREL_SHOTGUN_MAIN.getModel(), stack, matrixStack, buffer, light, overlay);

        if(Gun.hasAttachmentEquipped(stack, IAttachment.Type.STOCK)) {

            if (Gun.getAttachment(IAttachment.Type.STOCK, stack).getItem() == ModItems.MAKESHIFT_STOCK.get())
                RenderUtil.renderModel(SpecialModels.DOUBLE_BARREL_SHOTGUN_STOCK_MAKESHIFT.getModel(), stack, matrixStack, buffer, light, overlay);
        ...
        if (entity.equals(Minecraft.getInstance().player)) {
             ...
             */
            matrixStack.translate(0, 0, cooldown / 24);
            matrixStack.translate(0, 5.8 * 0.0625, 0);
            //Renders the moving part of the gun.
            RenderUtil.renderModel(SpecialModels.DOUBLE_BARREL_SHOTGUN_CHAMBER.getModel(), stack, matrixStack, buffer, light, overlay);
            //Always pop
            matrixStack.popPose();
