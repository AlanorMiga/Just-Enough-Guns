...
public class PumpShotgunModel implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemDisplayContext transformType, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrixStack, MultiBufferSource buffer, int light, int overlay) {
        ...
        RenderUtil.renderModel(SpecialModels.PUMP_SHOTGUN_MAIN.getModel(), stack, matrixStack, buffer, light, overlay);

        if(Gun.hasAttachmentEquipped(stack, IAttachment.Type.STOCK)) {

            if (Gun.getAttachment(IAttachment.Type.STOCK, stack).getItem() == ModItems.MAKESHIFT_STOCK.get())
                RenderUtil.renderModel(SpecialModels.PUMP_SHOTGUN_STOCK_MAKESHIFT.getModel(), stack, matrixStack, buffer, light, overlay);

        }

        if (Gun.hasAttachmentEquipped(stack, IAttachment.Type.BARREL)) {
            if (Gun.getAttachment(IAttachment.Type.BARREL, stack).getItem() == ModItems.SILENCER.get())
                RenderUtil.renderModel(SpecialModels.PUMP_SHOTGUN_SILENCER.getModel(), stack, matrixStack, buffer, light, overlay);
        }
        ...
        if (entity.equals(Minecraft.getInstance().player)) {
             ...
             */
            matrixStack.translate(0, 0, cooldown / 11);
            matrixStack.translate(0, 5.8 * 0.0625, 0);
            //Renders the moving part of the gun.
            RenderUtil.renderModel(SpecialModels.PUMP_SHOTGUN_PUMPY.getModel(), stack, matrixStack, buffer, light, overlay);
            ...
            if (Gun.hasAttachmentEquipped(stack, IAttachment.Type.UNDER_BARREL)) {
                if (Gun.getAttachment(IAttachment.Type.UNDER_BARREL, stack).getItem() == ModItems.LIGHT_GRIP.get())
                    RenderUtil.renderModel(SpecialModels.PUMP_SHOTGUN_GRIP_LIGHT.getModel(), stack, matrixStack, buffer, light, overlay);
                else if (Gun.getAttachment(IAttachment.Type.UNDER_BARREL, stack).getItem() == ModItems.VERTICAL_GRIP.get())
                    RenderUtil.renderModel(SpecialModels.PUMP_SHOTGUN_GRIP_VERTICAL.getModel(), stack, matrixStack, buffer, light, overlay);
                else if (Gun.getAttachment(IAttachment.Type.UNDER_BARREL, stack).getItem() == ModItems.ANGLED_GRIP.get())
                    RenderUtil.renderModel(SpecialModels.PUMP_SHOTGUN_GRIP_ANGLED.getModel(), stack, matrixStack, buffer, light, overlay);
            }
            //Always pop
            matrixStack.popPose();
