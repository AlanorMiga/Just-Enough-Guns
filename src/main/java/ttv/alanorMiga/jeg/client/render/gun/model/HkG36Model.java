...
public class HkG36Model implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemDisplayContext transformType, ItemStack stack, ItemStack parent, LivingEntity entity, PoseStack matrixStack, MultiBufferSource buffer, int light, int overlay) {
        ...
        RenderUtil.renderModel(SpecialModels.HK_G36_MAIN.getModel(), stack, matrixStack, buffer, light, overlay);

        //Renders the iron sights if no scope is attached.
        if ((Gun.getScope(stack) == null))
            RenderUtil.renderModel(SpecialModels.HK_G36_IRON_SIGHTS.getModel(), stack, matrixStack, buffer, light, overlay);

        if (Gun.hasAttachmentEquipped(stack, IAttachment.Type.STOCK)) {

            if (Gun.getAttachment(IAttachment.Type.STOCK, stack).getItem() == ModItems.TACTICAL_STOCK.get())
                RenderUtil.renderModel(SpecialModels.HK_G36_STOCK_TACTICAL.getModel(), stack, matrixStack, buffer, light, overlay);
            else if (Gun.getAttachment(IAttachment.Type.STOCK, stack).getItem() == ModItems.LIGHT_STOCK.get())
                RenderUtil.renderModel(SpecialModels.HK_G36_STOCK_LIGHT.getModel(), stack, matrixStack, buffer, light, overlay);
            else if (Gun.getAttachment(IAttachment.Type.STOCK, stack).getItem() == ModItems.WEIGHTED_STOCK.get())
                RenderUtil.renderModel(SpecialModels.HK_G36_STOCK_WEIGHTED.getModel(), stack, matrixStack, buffer, light, overlay);
        }
        ...
        if (entity.equals(Minecraft.getInstance().player)) {
            ...
            RenderUtil.renderModel(SpecialModels.HK_G36_EJECTOR.getModel(), stack, matrixStack, buffer, light, overlay);
            //Always pop
            matrixStack.popPose();
